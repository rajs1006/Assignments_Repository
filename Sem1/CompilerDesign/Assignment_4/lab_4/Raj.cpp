...
public:
  ...
  void handleInstruction(Instruction &I) {
    if (ReturnInst *Ret = dyn_cast<ReturnInst>(&I)) {
      ...
      if(Br->isUnconditional()){
		...
		for (Instruction &SuccI : *SuccBB){
			if (isa<PHINode>(&SuccI)){
				...
				// Store phiNode and Values in SmallVector. The reason not
				// to use SET for storing as SET does not maintain the order. 
      ...
      HandledInstrs.insert(Br);

      const char *operatorVar;
      ...
      if(Br->isConditional() ){
      	...
      	emitInstr(operatorVar, getInReg(Op1), getInReg(Op2), BlockNames[succ]);
      	emitInstr("j", BlockNames[Br->getSuccessor(1)]);
	  }
    } else if (LoadInst *Load = dyn_cast<LoadInst>(&I)) {
      const char *loadInstVar;
      Value *Op1 = Load->getPointerOperand();
      std::string Op1Reg = getInReg(Op1);
      // This OutReg is un-used but written here to match 
	  // the output Registry numbers with the one given in 
	  // .hint file.
      ...
      emitInstr(loadInstVar, ResInst, Op1RegLoad);
...
/**
 * This method assignes the Registry to Phi Nodes
 * while iterating over the SmallVector contains
 * pair of PhiNode and corresponding Value.
 */
 void phiNodeRegAlloc() {
	for (unsigned i = 0, e = PhiValues.size(); i != e; ++i) {
		...
		for (unsigned j = i + 1, e = PhiValues.size(); j != e; ++j) {
			...
			// If PhiNode has been used, assign register to the 
			// user phiNode first.
			if (phiNodeI == inValJ) {
  ...
  // This Set contains the phiNodes to avoid duplicate registry

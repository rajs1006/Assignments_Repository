...
def plot_identified_faces(scores: np.ndarray, training_images: list, test_images: list, pcs: np.ndarray, coeffs_test: np.ndarray, mean_data: np.ndarray, face_space_threshold,threshold : float=0.8):
    ...
    for i in range(scores.shape[1]):
        ...
        face_space = face_space_distance < (1 + face_space_threshold);
